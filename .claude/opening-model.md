...
`Opening.fixtureType`は元々「窓の材質記号」（`AW`/`JW`）だったが、「建具記号（材質×種別）」へ意味を拡張して`AD`/`SD`/`WD`/`SW`を追加した。既存FBS enum値（`AW=1`/`JW=2`）は温存し追記のみで拡張している（`.claude/serialization-fbs.md`の末尾追加ルール）。新規フィールドを増やさないことで、モデル層に「建具記号」概念を二重に持たない。本体記号以外（ユーザーがカタログ保守で追加した記号）は文字列フィールドへ書く——設計意図は`.claude/catalog-model.md`参照。
建具種別（`FITTING_CATALOG`/`WINDOW_CATALOG`）・建具記号（`FIXTURE_SYMBOLS`）のエントリはカタログの三層（同梱＞ライブラリ＞本体）で解決される——一覧を直接見ず`findCatalogEntry`/`openingSubTypeList`/`getFixtureSymbols`を経由すること。
...
**不変条件: 蝶番系の回転中心（＝扉が閉じる壁面）は「扉が開く側の壁面」に置く。** 扉は開く側の面で閉じ蝶番もその面にあるため、`host.axisValue`（ユーザーが叩いた面）へ固定すると「開く方向反転」後に閉じた扉の四角・方立の欠き込みが元の面に残り、扉が壁厚を貫いて反対側へ開く（実際に起きた不具合）。`planSymbolPlan`が`swingOpenPerpDir`（開く直交方向。両開き系は実効吊元-1）と面線`faceLo/faceHi`から`pivotPerp`と`leafOutward`（閉じた扉の厚み・欠き込みが壁中心側へ向かう向き）を決め、`OpeningsLayer.jsx`は消費するだけにする——`Math.sign(host.axisOffset)`で自前計算しない（hostは叩いた面の壁で開く側とも室外側とも一致せず、CL偏芯でも破綻する）。既定配置では開く側＝叩いた面のため従来と同じ位置になる。面線は略図LOD以外で引く（一般LODでも壁は厚みを持って描かれるため回転中心が要る）。
...
`IMPLEMENTED_MECHANISMS`（`openings/openingPlanSymbol.js`が参照）と`openingElevationFigure.js`の`mechanismPrimitives`は別々に機構をカバーし、揃っている前提を作らない。平面のみ実装済みの機構は姿図側で`entry.label`のテキスト表示にフォールバックする（クラッシュはしない）——姿図側の未実装は`IMPLEMENTED_MECHANISMS`から見えないため、姿図を追加するときは`mechanismPrimitives`のswitchへ機構を足すだけでよく、平面側の変更は不要。

## 平面記号の組立ては openings/openingPlanSymbol.js へ抽出（純関数。react-konvaを引かない）
`renderer/OpeningsLayer.jsx`は判断を一切持たない——host壁・面線・カタログentryを引いて`openings/openingPlanSymbol.js`の純関数へ委ね、返ってきたワールドmm単位のプリミティブ列をKonva要素へ機械的に変換するだけ（ステップ11「作図P2」・2026-09-23。`openingTagPlacement.js`⇄`OpeningTagLayer.jsx`と同じ分離。node:testから単体importできる）。線の太さは記号の役割（記号本体・枠材・扉本体・動作弧）で決まり、その決定を純関数側の1箇所に集約する——jsx側には持たせない。角度計算・leaf仕様決定・トラック配置等の純粋な数値計算は引き続き`openings/openingPlanSymbolGeometry.js`に置き、`openingPlanSymbol.js`はそれを呼んで機構ごとにプリミティブを組み立てる層。判断（太さの役割決定を含む）を`.jsx`側に残すと結線ミスがテストで検出できないため純関数側に一本化する——**不変条件: 2枚leaf構成の対向leafはswingSide（回転センス）を反転して渡す（反転しないと2枚が壁の反対側へ開く）**。
...
記号`WF`/`SF`/`SSF`は機構スコープ付き記号（`FIXTURE_SYMBOLS`の`mechanism:FRAME_ONLY`）——`getFixtureSymbols(category, mechanism)`の第2引数がFRAME_ONLYのときだけこの3件を返し、それ以外（省略時含む）は従来のスコープ無し記号のみを返す。種別変更時の記号切替は`openingEdit.js`の`fixtureTypeAfterSubTypeChange`（現在記号が新機構のスコープに無ければ新機構の既定記号へ差し替え）に一本化する。`getFixtureSymbols`は三層込み（ユーザー追加の記号も一覧に出る）。未知の記号（同梱のみに存在等）は建具種別を変えても保持し、黙って既定記号へ差し替えない。
...
## 建具の位置移動（ドラッグ・「位置」欄）は可動範囲へクランプ——弾かない
`openings/openingMove.js`（純関数）が可動範囲・スナップ・確定値を一手に持ち、平面ドラッグ（`usePointerInteraction.js`）と建具タブの位置欄（`resolveRefOffsetEdit`）が同じ範囲を使う（ユーザー裁定2026-09-14「範囲端へクランプ＋トースト」。幅編集の`maxOpeningWidthAt`と同じ「弾かず丸める」思想）。範囲が引けない（ホスト壁なし）ときだけ従来の`validateOpeningEdit`にフォールバックする。
可動範囲は「壁両端・隣接開口端・壁を横切る通り芯と中心線」で挟む——中心線もまたげない（間仕切りの芯をまたぐ建具は成立しない。補助線・梁芯は区画ではないのでまたげる）。境界の位置は**CL値ではなく、そのCL上で境界に突き当たる直交壁の材の面**（建具枠が壁に埋まらない。偏芯壁は`Wall.materialRange`で実位置）。直交壁は境界のどちら側から突き当たるものも数え、1つのCL上の複数壁（下地オーナー壁＋仕上げ薄壁）は材を合併する。今すでにまたいでいる旧データのCLは無視する（動かせなくなるより次のドラッグから拘束される方が実用的）。
スナップは閾値内の候補を「通り芯 → 中心線・補助線 → 範囲端」のティア順で選ぶ（同ティア内は最近傍）——実データでは通り芯が壁端の数十mm内側を横切り、範囲端・100mmピッチの中心線候補が同じ閾値内に密集するため、最近傍だけでは通り芯で止まって見えない。閾値・ステップ刻みはCL移動と同じ（`SNAP_THRESHOLD_PX`・`calcStep`）。
展開図でも同じ範囲・スナップで壁長さ方向へ動かせる——`elevationFigure.js`が建具ごとに姿図外形の透明ヒット矩形（`type:'hit'`。線分1本ずつのヒットは細線で実用にならない）を出し、記号丸と共に`openingId`/`dirSign`（画面x→世界座標の符号。`localXOf`の逆）をKonva属性として持たせる。`refOffset`が変わるたびに`ElevationModeState`の reaction が帯を丸ごと再構築するため、ドラッグ中は`refOffset`を書かない——確定値だけ求めて`setOpeningDragPreview`（その建具の`openingId`付きプリミティブだけを帯ローカルxでずらして描く。`ElevationLayer.jsx`）を requestAnimationFrame で更新し、確定時に1回だけ書く（建具の全プリミティブに`openingId`を付けるのはこのため）。

## 建具の袖柱は構造モード側の関心（在来木造のみ）
在来木造の`autoFillWoodColumns`（`structural/woodAutoFill.js`）が、全建具（三方枠FRAME_ONLY含む）の両袖にクリアランス付きで構造柱を自動生成する。開口の位置移動・削除に自動追従する（設計意図・AXIS導出・同一性キーは`.claude/structural-model.md`「建具の袖柱」節）。`Opening`側に新しいフィールドは増えない——柱側が`woodJambRef`で開口idを参照する片方向の関係。
