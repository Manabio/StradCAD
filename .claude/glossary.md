...
## 建具記号（fixtureType）
建具・窓の材質×種別を表す記号（`AW`/`JW`/`SW`/`AD`/`SD`/`WD`）。`Opening.fixtureType`の意味拡張（旧「窓の材質記号」→「建具記号」）で表現し、別フィールドを追加しない。記号ごとに独立して`記号-連番`（例`AW-1`）で採番する。ユーザー追加可・本体記号以外は文字列フィールドに書く。設計意図は`.claude/opening-model.md`。

## カタログ／本体・ライブラリ・同梱（builtin/user/doc）
**カタログ**＝材料・内装マスター・境界マスター・断面・建具種別・建具記号の6種別（`catalog/catalogKinds.js`の登録表）。**本体（builtin）**＝アプリのソースに書かれた既定値、**ライブラリ（user）**＝アプリ単位でIndexedDBに持つ追加・編集分、**同梱（doc）**＝文書ファイル（.stq）に入った分。解決順はdoc＞user＞builtin。設計意図は`.claude/catalog-model.md`。

## 標準を編集（上書き）
本体（builtin）のカタログエントリを、同キーのライブラリ（user）エントリ＋`overridesBuiltin`印で上書きすること。新しい仕組みは作らず既存の衝突検出・重複禁止をそのまま利用する。設計意図は`.claude/catalog-model.md`。

## 読み替え表（aliases）
カタログキーの読み替え（読込み時の内容完全一致による自動読み替えと、指示UIでの承認・ピック）を記録する文書固有の表（束の`aliases[kind]`）。本体側のコード振り直し表（`legacyMaterialCodes.js`）とは別物。設計意図は`.claude/catalog-model.md`。

## 合わせ直す
文書同梱（doc）のカタログエントリを、内容が食い違った本体・ライブラリ側の値へ差し戻す操作。全編集タブ（材料・内装・断面・建具種別・建具記号）で提供（境界マスターは閲覧のみ）。設計意図は`.claude/catalog-model.md`。

## 未解決（変換先の指示）
指示UI（`ui/CatalogResolveDialog.jsx`）に並ぶ、カタログ参照を自動では解決できなかった行。場面は4つ（ライブラリとの衝突・参照先の無い未解決コード・未対応・本体への追加候補）。既定の決定は候補の先頭を承認、追加候補だけは保留。選択肢は承認・代替を指示・保留が共通で、追加候補だけライブラリへ追加、ライブラリとの衝突だけ本体の編集として扱える。設計意図は`.claude/catalog-model.md`。
...
## 昇格/降格/削除問題
通り芯/中心線相互の昇格・降格、通り芯の削除（および移動・追加）に絡む一群の既知の限界の総称。
包括的なルールと制限が今後必要という前提のもと、個別の場当たり的な特例を積み増さず「既知の限界」として
受容する方針（2026-09-25ユーザー裁定）。一覧・設計の記録は`.claude/cl-conversion-limits.md`。
