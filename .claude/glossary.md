...
**梁**＝2点間で荷重を支える横架材（梁天端FL−100・材幅は柱同寸）。**受梁**＝当該階の柱の下階に柱のない梁（受梁を受ける梁は受梁同寸）。**床梁**＝上下階とも壁・柱は無いが根太を受けるため柱間・梁間に1820を超えない位置に設ける梁。**頭つなぎ**＝上階に壁は無いが下階の壁上にある横架材（材の性格は梁同等）。**火打ち梁**＝梁の交差する隅角部（四隅）に斜めに架け床の水平剛性を保つ材（16㎡以下の四角の4隅。吹抜けは可、EV・階段内は不可）。値は`structural/structureRules.js`の`TRADITIONAL_WOOD_FRAMING`、適用は`woodFraming.js`。
