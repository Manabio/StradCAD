...
## localStorage への「単一グラフ退避」は持たない——退避先はUndo（メモリ）とfloors（IDB）だけ
かつてハンバーガーメニューにあった「読込み/書出し」は、アクティブ階1枚を`localStorage`のキー1枠へbase64退避する文書ファイル以前の経路で、廃止した。理由は、退避元の階IDを持たないため階を切り替えてから読み込むと他階へ流し込む・1枠上書きで世代がない・localStorage容量上限に当たる、に加えて「開く/保存」とラベルが衝突していたこと。一時退避の役割はすでに**Undo（`undoManager`がメモリ上に持つ`serializeGraph`バイト列。記録時のモード・階も添付）と`floors`ストア（階切替のスワップアウト）**が担っており、`localSnapshot.js`のlocalStorage経路はどちらにも使われていなかった。以後、退避目的で`localStorage`へグラフを書く経路を足さないこと（残骸キー`strad-autosave`の掃除だけ`clearLocalAutosave`が担う）。
