...
（実測: 13.stqの2a壁は全てisRoomWall:true）。`sectionProbe.js`（列の分割）と`sectionHits.js`
（候補収集）の**両方**で外す（片方だけだと壁は消えても壁端で列が割れたまま残り、そこに縦線が出る）。
旧実装はfootprint外接矩形の辺に載る2a壁（実機「13」のy=−3500の2枚）を軸CL値だけの判定で残して
いたが、それは階段室の壁ではなく部屋13の2a壁で、新方式では規則どおり隠れる（13.stq「6」面C:
アキが帯内x 0〜1505→0〜2885に広がり、z=2400の破線とz=3000の線が2885まで伸びる。ユーザー裁定待ち）。
...
効き方が3つ同時なのが要点: レーン境界（中心1）に載る2a壁は「その切断で最も手前の壁面」になるため、
(1)壁端の縦線が出る、(2)その先の列が奥行き上限（`SIGHTLINE_DEPTH_LIMIT_MM`）を超えて`open`になり
アキのバツが出る（本来はもっと奥の壁が見えがかりとして続く）、(3)`isBlockedByWall`が成立して
復路ささらの見えがかりが消える。**階段下に部屋が無いと2a壁自体が生成されないので3つとも起きない**
——「階段下の部屋の有無で描画が変わる」症状の実体はこの壁1枚である。
