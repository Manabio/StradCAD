...
**階段下部屋の2a壁は、階段の帯からは実体ごと見えない**（`switchbackCuts`の`stairUnderInfo`が
`room.generatedWallIds`のうち**階段footprintの内側に立つCLのもの**だけを`cut.hiddenWallIds`に載せ、
`sectionProbe.js`が候補収集と列の分割の**両方**で外す。ユーザー実機指摘2026-09「「6」D1: 「13」の壁関連
（2本の縦線とアキばつ）は、階段より下なので描画しない」）——階段下の空間の壁で、その展開は
階段下部屋自身の帯（「13」A〜D）が描く。平面図が破れ線より階段踏面側で2a壁を描かないのと同じ規約
（`finish/stair/stairUnderClip.js`）。**footprint境界上のCLは除く**——階段下部屋の外周のうち階段室
自身の外周と重なる辺は階段室の実壁そのもので、階段の展開図に必要だから。
...
列の分割からも外すこと（片方だけだと壁は消えても壁端で列が割れたまま残り、そこに縦線が出る）。
