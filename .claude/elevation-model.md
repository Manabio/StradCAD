...
1枚の帯へ**2つのエンジンが同時にプリミティブを流し込む**（`elevationVoid.js`は同じ帯に対し
`layoutBandFaces`→`buildFaceFigure`と`buildCutContent`→断面エンジンの両方を呼ぶ）。境界は次のとおり。
...
| 仕事 | 図面の体裁（開口・巾木・壁材2段書き・寸法・通り芯丸・面ラベル・アキ・段差見付け）＋自階の輪郭 | 空間の見え方の解決 |

**新しい表現を足すときの判断**——自階で閉じる表現は`buildFaceFigure`、他階・遮蔽が絡む表現は
断面エンジン、そして**壁の実体に属する表現（天端・端部・材の縁）は両方に要る**。3列目を忘れると
「単層の帯だけ正しく、多層書きされた上階だけ線が足りない」という**例外もログも出ない欠落**になる
（実機2026-08の腰壁天端がこれ。1階の帯に多層書きされた2階の腰壁に天端が出なかった）。
両方に要る規則は片方へ書かず、共有できる形（例: `elevationStyle.js`の`kneeCapBottomMm`）で1箇所に置く。
長期的には「壁の輪郭」だけを断面エンジンへ寄せる方向（階段帯・上部吹抜けは移行済み。残るは
通常の部屋帯の自階の輪郭）だが、`buildFaceFigure`の輪郭にはユーザー裁定由来の規則が多数
埋まっており視覚回帰のテストが無いため、機能追加のついでにやる規模ではない。
