...
`face.spans`（`elevationOpenSpan.js`）の**遠側**床線・天井線だけは`buildFaceFigure`に残っている
（**その上のアキは2026-09に移行済み**。下記「アキのz範囲」参照）。断面エンジンは層ごとの床天井を**切断面の位置の部屋**から解決する
...
高低差への追従（上端は近側/遠側の天井の低い方で止まる。下端は**遠側床が帯の床より高いときだけ
そこまで持ち上げ、それ以外は探査が見つけた床のまま**）に要る遠側のFL/CHは
探査からは得られないため、`cut.openSpans`として呼び出し側（`appendBandCutContent`）から渡す
——判定材料が主cutに無いものは呼び出し側から受け取る、という`aboveCeilVisibleRanges`と同じ形。
**値の単一情報源は`face.spans`・描画の単一情報源は断面エンジン**なので、図側に残る遠側床線と
アキの下端が同じ値から出て食い違わない。
...
**「ア キ」の中心もクランプ後の範囲で採る**。連結成分に複数の開放スパンが掛かるときは
**重なりが最大**のスパンのfar値を使う（端が僅かに掛かった別スパンを拾わない）。
...
分割だけ**（下記`splitOpenByFarFace`の深度上限適用・見えがかり線描画）。アキ下端のクランプ
規則（下記`spanLoZ`。2026-09-11裁定）はこのフラグの外で常に新規則のまま動くため、offにしても
13.stq「10」面D1のアキ下端は新規則(z=0)のまま——**offはPhase 4前の基準ではない**）:
...
何もしなくてよく、水平面ヒットの見えがかり線・アキの縮小だけを比較したいときはダンプ側で
`dumpElevFigure.mjs --no-horizontal-faces`を付ける（アキ下端のクランプ規則は下記「裁定済み
（採用。2026-09-11）」のとおりフラグの外なので、このoffでは戻らない）。

**裁定済み（採用。2026-09-11）**: アキの下端の既存確定裁定（`:295-296`の旧文言「下端は**遠側床が
...
ユーザーはPhase 4側（常に縮める）を採用と裁定し、`:295-296`の文言と`sectionEmit.js:1312`
（`spanLoZ`。`cut.openSpans`のfar値が`null`でなければ常にクランプ）を同時に揃えた——
片方だけ変えるとPhase 5（`openSpans`注入を探査の答えへ差し替え）のゲート「diff空（一致しなければ
Phase4が誤り＝検算）」が食い違って落ちるため。
**残る非対称（未解消・Phase 5で検算対象）**: `splitOpenByFarFace`はfarが区間の**内部**
（`z0<far<z1`）のときだけクランプするが、`spanLoZ`はfarが有れば区間の外（`far<z0`）でも常に
クランプする。3点表1行目「11'」A2（far=−100・探査z0=0。far<z0で区間の外）はこの非対称の実例
——`spanLoZ`はz0=0からz=−100へ持ち上げる（表のとおり正解）が、`splitOpenByFarFace`はfarが
区間の外なので同じ入力では縮めない。Phase 5で`openSpans`注入を撤去するときはこの差がdiffとして
出る想定。
