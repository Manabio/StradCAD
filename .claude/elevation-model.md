...
**レーン区間の1FL線は「階段断面に出会ったら終点」**（`floorSegments[].flatLineSpanX`。区間の床線を
描く範囲を狭めるフック。既定=未指定=区間いっぱい＝従来どおり）——階段下に部屋があるとき、レーンの
床は階段そのもので、1FLの水平線は**上り口の外側（壁のない端部のはり出し）にだけ**残る。終点は
上り口端±段鼻の出（`flight.nosingMm`）で、断面ジグザグの始点とちょうど繋がる。
旧実装の`hideFlatLine`は区間を丸ごと非描画にしており、はり出しごと消えていた（ユーザー実機指摘
2026-09「「6」D1: 1階Y2から3500、下りた階段の先（左側）に1FL断面はり出しが正解」）。
