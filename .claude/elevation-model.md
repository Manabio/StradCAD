...
**`reorientFace`はletter（展開記号）もdirSignから引き直す**（`letterForDirSign`。ユーザー実機指摘2026-08）
——記号A/B/C/Dは視線の向き（A=12時/B=3時/C=6時/D=9時）＝図の左→右がどちらの世界方向かで決まるため、
`DIR_SIGN[letter] === face.dirSign`は面の不変条件。旧実装はletterを「同じ物理壁を指す不変な性質」として
据え置いており、実機で「左手に登り口・右が踊り場＝9時方向を見ている図なのに記号がB（3時）」になっていた。
