...
**巾木の端も床線と同じ描画基準（`drawnX0`/`drawnXRun`）で引く**（`elevationFigure.js`。
ユーザー実機指摘2026-08「「5」D1・B1: 1階巾木、CLで終わらず、開いている壁にはね出し」）——
巾木の区間端は論理境界（`segs[i].loX/hiX`＝CL）を使っており、壁のない端部で床線だけがはね出して
巾木がCLで止まり、段が付いていた。内部の境界を`riserXAt`（床が低い側へ半壁厚）で引くのと同じく、
**巾木は描画要素なので端も描画基準に揃える**（開口・開放スパンでの途切れは従来どおり）。
...
アキの下端が同じ値から出て食い違わない。
**下端をfar値で上書きしてよいのは「遠側床が帯の床より高い」ときだけ**（ユーザー裁定2026-09。
実機の3点で確定した規約で、1点だけ合わせると他が回帰する）:
| 実機 | far | 探査が見つけた床 | 正しい下端 |
|---|---|---|---|
| 「11'」A2左（遠側が一段上の床） | −100 | z=0 | z=0（＝一段違う床の線） |
| 「10」D1（近側の床が部分指定で下がる） | 0 | z=−50 | z=−50 |
