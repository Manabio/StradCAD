...
アキの下端が同じ値から出て食い違わない。
**下端＝その位置で実際に描かれている遠側床の断面線**（ユーザー裁定2026-09「バツは破線=1FLまで」。
実機の3点で確定した規約で、1点だけ合わせると他が回帰する）。遠側床が帯の床と**異なる**ときだけ
遠側床へ着ける——同値なら遠側床線はそもそも描かれないので探査の値のままにする:
| 実機 | far | 探査が見つけた床 | 正しい下端 |
|---|---|---|---|
| 「11'」A2左（1FLの破線が引かれている） | −100 | z=0 | z=−100（＝その破線の高さ） |
| 「10」D1（遠側床線は無い・近側の床が部分指定で下がる） | 0 | z=−50 | z=−50 |
...
採取元は`D:/tatsuya/Download/13.stq`・採取時点はコミット5943d89（移行前）——以後このスナップ
ショットは再生成しない（動かないゴールデンでなければ「新設計との差分」を測る基準にならない）。
