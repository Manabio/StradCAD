...
**アキのバツは手前の階段に隠れる区間だけ破線**（`splitGapMarksByStair`＋`stairOccluderRects`。
ユーザー実機指摘2026-08「6」C「但し、階段に隠れる部分は破線」）。隠れる範囲はプリミティブからの
逆算ではなく**flight自身のモデル値**——見付け幅（梯子と同じ`ladderAcressRange`調整済み。STEEL時）と
昇り切り高さ（`baseZ`〜`baseZ+steps*riserMm`）——から矩形として組み立て、対角線をLiang-Barskyで
区間分割する。対象は`dash:'center'`の斜め線だけ（既に`dashed`のもの＝床断面より下のアキ、水平・垂直線は
素通し）。レーン縦断（側面視）の切断は対象外——レーンが切断線の中を通っており「手前に見える階段」では
ないため（`crossesFlight`がfalse）。
