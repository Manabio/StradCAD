...
- **診断結果（2026-09-12）— 3つの根本**:
  ...
  - **根本C（B の派生）**: `computeFlightProfile` の `clamp` が `fullColumnsXRange` へ**点を切り詰める**ため、最終段の蹴込オフセットが失われ斜め線が垂直に潰れる。→ D2-2。一般解: 点の切り詰めを線分の交差クリップへ。既存仕様「最終段も同じループで処理、例外なし」からの逸脱＝バグ。
    ...
    - **P1〜P3 完了・コミット 9b36c33 / 3a04a3b（2026-09-13）**。QA 3ラウンド合格。出力変化は 13/11「6」の3本のみ。
    - **ユーザー裁定（2026-09-13 その2）: §5.12 の6点指摘のうち未完のものは全てキャンセルし、不備を許容したまま当初計画（問題.md＝§5.5 の Phase 表）の完遂を優先する。** キャンセル対象: P4（D2-3。下ささらを断面線でクリップ）、P5（84mm片）、Q1〜Q4 の未確認事項、C-1後半/C-2後半/D2-1前半の未確認分。golden13/golden11 の「6」は現状の出力（P3 後）で再採取して以後のゲートにする（再採取の可否は実機確認との順序を裁定）。
    - **裁定（2026-09-13）: 6b-3（stairOver）は当初計画どおり実施／golden は実機確認前に現状出力で再採取してよい。** 再採取実施: 旧 golden との差分は「6」（Phase 6前半〜設計(d)）と「5」「22」（コミット 5d2ef84 の裁定済み差分。golden 未反映だったもの）。新 golden と現状ダンプは一致。
    - **6b-3 設計（architect 2026-09-13）と裁定**: `stairOver` は `elevationBand.js:690-767`（§1.3/§5.11 の行番号は行ずれ）。P0 実測: 「13」の stairOver 経路の列は全列 `wall[0..2400]` 1本のみ（slab/cut/open ゼロ）→ `solidRectsOf` は空＝#4 天井クリップを一般判定で置換すると大回帰（不可）。側面視の面B/Dで `stairFace` ヒット0件（6b-1 が斜め踏面ジグザグをスコープ外にしたため）→ **§5.11 の「#7 面端縦線＝遮蔽チャネル」「#8＝cellAt」は誤り**（13 の天井は全セル 2400 一定）。置換可＝#1（`crossesFlight`/`isLengthwiseCut`）、#2/#4 の天井基準（`layout.CH`→`cut.ceilProfile`）、#8（重心→交点区間ごとの判定）。残す＝#3 生成・#4 クリップ本体・#6 踊り場線・#7（`lowerFaceEndVertical`＝図の線を後から書き換える唯一の残存機構。クリーンアップ Phase の対象）。**ユーザー裁定: (A) 出力不変の3点で 6b-3 を切り上げ Phase 7 へ**（(B) 踏面ジグザグをヒット列へ載せる 6b-1′ は当初計画外のため不採用）。段階: 3a cut に `stairCut` を渡す→3b #1→3c #2/#4 天井基準→3d #8。全段 golden13/11/knee diff 空。
    - **6b-3 実装結果と裁定 X（2026-09-13）**: 3a/3b/3c は golden 不変で通ったが、3d（#8 重心→交点区間）は golden13/11 で面B/D の天井線が消え revert（原因: クリップ後の CUT 点群が交点の両側にまたがり「区間内に1点でもあれば占有」が誤判定。正しくは `stairZAtRun` で区間ごとに判定し直す必要＝再設計課題）。QA: 3c の `stairHiddenByCeil` 新基準（`cut.zRange.hiZ`＝区間最大）は 13/11 で旧値と恒等＝一度も実行されず、テストも無し（F1）／stairOver 内に天井基準2系統が同居（F3）／3b は判定の狭めで実データに無い構成で出力が変わりうる（F4）／コード行 51→51 で削減ゼロ（F6。`flightRect`/`rectsOverlap` は `flightsOverRoom` が使うため削除不可）。**ユーザー裁定 X: 検証できる範囲に縮小**——残すのは 3a（`stairCut` を cut に渡す）と、エンジン内の単一情報源化（`sectionEngine.js` の私設 `ceilZAt` を削除し `sectionTypes.js` の `ceilProfileZAt` へ）。**3b と stairOver 側の天井基準変更（`stairHiddenByCeil`/`clipStairUnderCeiling` の profile 化）は revert**。stairOver の残り判断（#1/#2/#4/#5/#7/#8）は**踏面ジグザグのヒット列化（6b-1′）が前提**で今は置換不能＝クリーンアップ Phase へ送る。Phase 6 は「hiddenWallIds 側の置換完了・stairOver 側は前提欠如で保留」として閉じ、Phase 7 へ。
    - **Phase 7 設計（architect 2026-09-13）と裁定**: 層構成のリテラルは8箇所（`elevationBand.js:813`／`elevationVoid.js:536,666-669,191,486-492`／`switchbackCuts.js:346-347`／`straightCuts.js:107-108`／`elevationSolids.js:48-50`）。探査側は N 層耐性あり（INV2 の3層全順列テスト既存）。N 層で壊れるのは `elevationStairSequence.js:690`（非自階層を配列順先頭で拾う）のみ。**当初案「`componentOf` で層を列挙」は不成立**（①開口でつながる隣室の上の吹抜けが通常帯に above 層を生やす ②階段帯は吹抜けの有無に関係なく above を積む ③吹抜け帯の below の `-dropMm` はアンカー室の実効FL差込みで連結では出せない ④78ms/回）→ 連結は可視判定側に留め、**層は呼び出し側が持つ階リストを宣言的に渡す**。設計: 新規 `section/sectionLayers.js` `buildBandLayers(selfGraph, {above:[{graph,floorHeightMm}], below:[{graph,floorHeightMm,anchorRoom}], selfFloorZMm})`（規則4つ・帯種別 if ゼロ・返り順 self→|Δz| 昇順固定）。階段の cut 表は層を受け取る形へ（Phase 8 と同じ向き）。`heightUnits = layers.length` に統一（現行値と全帯一致を確認）。段階: 7a-0 基準再確認→7a-1 新設＋置換→7a-2 階段 cut 表→7b-1 heightUnits→7b-2 `:690` 是正＋3層合成 fixture テスト（全段 golden 3本 diff 空）。**ユーザー裁定: (A) 入口の一般化まで（出力完全不変）を Phase 7 とし、(B) 実データで3層を描く（N階 peek・吹抜け連鎖・多段化・3階建てテスト .stq・実機裁定）は Phase 7c として別フェーズに追加**（Phase 8 の前後は別途）。
    - **Phase 7 (A) 実装完了（2026-09-13。QA 1ラウンド）**: `section/sectionBandLayers.js` `buildBandLayers` へ層リテラル8箇所を統合（残る `role:self` リテラルは1点プローブ用の `elevationFaces.js:243`/`switchbackCuts.js:91` のみ＝帯のスタックではなく対象外）。階段の cut 表は `opts.layers` 必須（フォールバック無し。テストは `buildBandLayers` を渡す）。`heightUnits = layers.length`（不変は golden ではなく **base 比較**で確認: 13/11/knee 全41帯の heightUnits/unitHeightMm/heightMm が SAME）。`layerDirectlyAboveSelf` は `sectionLayerStack.js` の `baseLayerOf`＋`layersAboveOf` のアダプタ（role 名依存を再導入しない）。3層合成 fixture のテスト（配列順入替で同結果）。golden13/11/knee 一致、test 2148。**完全不変の例外**: 階高が非有限（NaN）の入力でのみ旧=2層／新=1層（実データでは到達しない。コメントに明記）。 コミット fe57a6d / da1ce2c。
    - **ユーザー裁定（2026-09-13）: 次は Phase 8（面非依存の入口）を 7c より先に**（当初計画の順・出力不変の新規パスで進められる・Phase 7 の layers をそのまま入力にする）。→ **Phase 8 設計（architect 2026-09-13）と裁定**: 「`buildCutContent` は面を要求しない」は内部まで成立（`cut.face` を読む3箇所 `sectionContent.js:183-184,121,259-260` は全て null ガード済み）→ **新規パス並置ではなく既存帯経路の載せ替え（出力不変）**。契約: `buildSectionFromLine(line, layers, opts)` を `sectionContent.js` に追加（新規ファイル・依存ゼロ）。`face` は opts の「面由来の値の供給元」に降格。面固有の値（`viewSign`/`dirSign`/`zRange`/`baseFloorZ`/`ceilProfile`/`floorZProfile`/`aboveCeilVisibleRanges`/`stairCut`/`airRoom`/`underRooms`/`bandRoomBounds`/`probeCtx`/`endExtendMm`/`upperPlaneOverhang`/`scale`）は呼び出し側が渡す（`zRange`/`baseFloorZ` をエンジンに導出させると帯種別 if が生えるため導出しない）。新設キーは `ends={openLo,openHi,wallLessLo,wallLessHi}` のみ（面述語の導出を1箇所へ。`withProbeExtension`/`emitCtxForCut` は `cut.ends` を読む。`cut.face` 読み取りは3→1＝`layerRunWindowsOf` のみ。face-less では層ごとの窓は作らない＝正しい縮退）。返り値は `buildCutContent` と同型（SectionFigure ではない）。**3本目の入口は作らない**: `buildSectionFigure`（直進階段専用の2本目 `sectionEngine.js:407`）の統合はクリーンアップ Phase。載せ替え対象は `elevationBand.js:649-690 appendBandCutContent` と `elevationVoid.js:232-245 appendUpperStoreyOutline` の2箇所。階段 cut 表は触らない（恒等変換で差分リスクのみ）。**7チャネルの現状**: #4 openSpans（Phase 5）・#5 drawFloorProfile（6b-2）が消え、現在6本（1/2/3/6/7＋6b-3 の stairCut）。Phase 8 で新設なし。**範囲外**: 斜めの切断線（軸平行前提）／多室横断での `baseFloorZ`・`ceilProfile`（後者は根本A と同じ問い）／材の層構成／体裁／`buildSectionFigure` 統合。段階: 8a-0 基準→8a-1 新入口→8a-2 elevationBand 載せ替え（最も差分が出やすい）→8a-3 elevationVoid→8b 合成 fixture 3本（部屋中央横切り／2室横断／失敗系）→8c（任意）`dumpSectionFromLine.mjs`。**ユーザー裁定: face-less 切断の既定は `bandRoomBounds` を渡さない（深度上限 800 のみ）**（根拠: `withinViewRoom` は 2026-08「6」C の 6m 先の壁対策として帯＝1部屋前提で入った特例。同時期の一般規則 `SIGHTLINE_DEPTH_LIMIT_MM` で同症状は落ちる。帯経路は渡し続けるので golden 不変）。
    - **Phase 8 実装完了（2026-09-13。QA 1ラウンド）**: `buildSectionFromLine` を `sectionContent.js` に追加、`appendBandCutContent`/`appendUpperStoreyOutline` を載せ替え。QA が base 比較で **198 cut 全てのキー集合・値の恒等**を確認（差は吹抜け上階経路の `stairCut` undefined→null と `endExtendMm` undefined→0 のみ＝消費点で同値）。builder が自己発見した `cut.line.axisValue`→`pcut.line.axisValue` は `withProbeExtension` が `axisValue` を書き換えないため同値（198 cut で実測、stairOver ブロックも実行経路上）。QA 指摘: JSDoc の「`layerRunWindowsOf` だけが `cut.face` を読む」は事実誤り（`sectionStructure.js:166,299`・`elevationStairSequence.js:567` も読む）→訂正／「真の面非依存」は過剰主張→縮退規則を明記／`baseFloorZ` 既定が未検証→テスト／死にパラメータ `floorOffsetMm` を削除。テスト9本（部屋中央横切り・2室横断・失敗系・ends 単一導出・face-less 縮退・baseFloorZ・zRange 必須）。golden13/11/knee 一致、test 2157。8c（`dumpSectionFromLine.mjs`）は未実施（任意）。
