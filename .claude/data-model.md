...
降格で複製した同一idのCLは「同じ線の分身」として扱う（2026-09-17裁定）: `findFloorsWithCounterpartCL`は`other.id !== cl.id`を条件に含め、複製自身を重複報告の対象から除外する（除外しないと降格→昇格の往復が塞がれる）。昇格成功後は非アクティブ全階にある同一idの複製を`recallPromotedCenterLineDuplicates`で回収する（`releaseCenterLine`のみを使う——`removeCenterLine`は壁を道連れ削除するため禁止）。回収は昇格が確定した**後**に行う（先に回収して途中失敗すると、複製を失った階の壁がrestoreGraphで捨てられる）。回収に失敗しても、降格→昇格を繰り返せば`propagateDemotedCenterLine`が改めて複製を作り直すため自己修復する——幾何自体は変わらない。
