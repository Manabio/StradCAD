...
（`closeConvexCorners`）。相手がそこで**終わる**（角）ならこれは正しいが、相手が**両側へ続く
通し壁**なら角ではなく、伸びた端の面線が通し壁の材を横切って反対側の面（CLを挟んだ向こう側の
仕上げ面）まで描かれる。`wallJunctionResolve.js`のパス6が、端が通し壁の材へ入り込んでいる場合
だけその材の区間を`spanCuts`で落とす——突き抜けている（X字）配置は端ではないので対象外、高さが
違う組はパス0の担当（低い壁の帯を覆うのは意図した振る舞い）。実機では1階X4通りの2か所だけが該当。
