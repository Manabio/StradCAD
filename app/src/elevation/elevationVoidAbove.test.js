...
test('【実機指摘2026-09】壁のない端部: 床線・天井線ははね出し、巾木は面の端で止まる（CLでは止めない）', () => {
  const cellKey = (g, x0, y0, x1, y1) => [
    clOf(g, CenterLineType.VERTICAL, x0).id, clOf(g, CenterLineType.HORIZONTAL, y0).id,
    clOf(g, CenterLineType.VERTICAL, x1).id, clOf(g, CenterLineType.HORIZONTAL, y1).id].join(':');
  const g1 = makeGraph('1階', 0);
  ...
  // **床線のはね出しは従来どおり**（ユーザー明示指示2026-08「天井と床の延長はこのままで良い」）。
  // このフィクスチャの先頭パネルはy=-3500の壁を落とした「壁のない端部」で、床線は-150まで伸びる。
  assert.equal(floor[0][0], -150, '壁のない端部では床線が150mmはね出すはず（この主張は変えない）');
  // **巾木ははね出さない**（ユーザー実機指摘2026-09「「11'」B1: 壁エッジ外に巾木の線が出ている。
  // 巾木は壁のある所にしかない」）——ただし止まるのは**面の端(local 0)**であって、CL
  // （壁中心線）ではない。CLで止めると、はね出した床線との間に段が付く旧不具合の再発になる。
  assert.equal(base[0][0], 0, '巾木は面の端(0)で止まるはず（-150へはね出さない／CL=-57.5でも止めない）');
  // 巾木がある区間の**もう一方の端**は従来どおり床線と揃う（内側の端は動かない）。
  for (const b of base) {
    assert.ok(floor.some(f => Math.abs(f[1] - b[1]) < 1e-6 || Math.abs(f[0] - b[0]) < 1e-6),
      `巾木の端は床線の端（またははね出し前の面の端）と揃うはず`
