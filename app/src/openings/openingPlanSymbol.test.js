// openingPlanSymbol.js（建具モード 平面記号の純関数化）の回帰テスト。ステップ11a。
//
...
function makeOpening(overrides = {}) {
  return {
    ...
    width: 1000, frameDepth: 0, lineWeight: 0.35,
    ...overrides,
  };
}
...
test('失敗系: DETAIL・実装済み機構(SWING)・frameDepth=50はnullを返し、exteriorDirOfも呼ばない（暫定契約の副作用ゼロ）', () => {
  const opening = makeOpening({ frameDepth: 50 });
  let calls = 0;
  const ctx = makeCtx({
    entry: { mechanism: OpeningMechanism.SWING }, lodLevel: LodLevel.DETAIL, faceLo: -60, faceHi: 60, axisValue: 0,
    exteriorDirOf: () => { calls += 1; return 1; },
  });
  const prims = buildOpeningPlanSymbol(opening, ctx);
...
// ================================================================
// (d) STANDARD/DETAILの実装済み機構はnull（一時契約。11b以降で機構ごとに置き換える）
// ================================================================

test('暫定契約: STANDARD・実装済み機構(SWING)はnull', () => {
  const opening = makeOpening();
