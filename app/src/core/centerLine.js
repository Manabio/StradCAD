 ...
 *
 * **`cl.labeled` だけで判定してはいけない**——旧UI経路で作られた旧データには `labeled:true` かつ
 * `discipline:ARCH`（中心線）の組合せがありうる（現行の `transform/centerLineOps.js`
 * `addCenterLineFromDialog` は kind:'center' 生成時に `labeled:false` を明示するため発生しないが、
 * 旧データ互換のガードとして本関数は種別（centerLineKind）まで見る）。`labeled` だけでは
 * この旧データも通り芯扱いになってしまう。
