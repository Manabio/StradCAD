...
export function roomCeilingHeight(graph, room, _depth = 0) {
  ...
  // 項目5: 部分指定（referenceRoomIdsで親を参照）が自身のCH指定（master/override）を持たず、
  // かつ親とfloorLevelが異なる場合は、天井面の絶対高さを親と揃えるようCHを段差ぶん増減する
  // （部分指定CH = 親CH − (部分FL − 親FL)）。自身に明示的なCH指定（master/override）があれば
