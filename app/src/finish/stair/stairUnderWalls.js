 ...
 *
 * 対象部屋の外周エッジ全周（破れ線・踊り場側を含む）に、下地を室内側へ全寄せした偏芯壁を
 * 生成し、外側セルが部屋（階段ペアRoom含む）に属する場合はその部屋の仕上げのみの薄壁を
 * 併せて生成する（.claude/stair-model.md・タスク仕様のルール1・2）。破れ線・U字系の踊り場
 * ストリップ境界（＝破れ線の「正面」）も同じルール1・2の経路に乗る（外側は階段ペアRoomの
 * ため薄壁は階段仕上げ厚になる）。U字系のレーン間中心線には専用の偏芯壁＋階段側仕上げ薄壁を
 * 生成する（ルール6）。
 *
...
import {
  ...
  computeExternalEdgeParams, findOutsideRoom, mergeSegments, clipToAxisExtent,
 ...
 */
export function stairUnderClaimedEdges(graph, room) {
  const unitParams = computeExternalEdgeParams(room, 1, graph);
  if (unitParams.length === 0) return [];
  return computeClaimedEdges(graph, unitParams);
}
 ...
 * @param {{dimsOf?: (room:import('@core').Room) => {wallBase?:number, wallFinish?:number}}} opts
...
export function generateStairUnderWalls(graph, stair, room, dims, opts = {}) {
  ...
  const unitParams = computeExternalEdgeParams(room, 1, graph);
  if (unitParams.length === 0) return { walls: [], claimedEdges: [] };

  const uCtx = (stair.type === StairType.SWITCHBACK || stair.type === StairType.WINDING)
  ...
  for (const p of unitParams) {
    ...
    // に属するため、findOutsideRoom がそれを解決しルール2の薄壁（階段仕上げ厚）が付く
  ...
  const claimedEdges = computeClaimedEdges(graph, unitParams);
