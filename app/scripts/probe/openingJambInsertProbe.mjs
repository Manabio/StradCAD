// 建具の確定・削除直後に自階の構造が自動再計算されること（openingStructuralSync.js）の
// 実データ確認用probe。tategu-test3.stqの縦内壁（X1+3640, Y1-1820付近。y は負方向）に
...
// （QA指摘: probe自身が openingStructuralSync.request を直接呼ぶと、本番配線
...
// recomputeStructuralForGraph/openingStructuralSync.requestを直接呼ばない）。
//
// 確認する事実:
...
import { openingStructuralSync } from '../../src/structural/openingStructuralSync.js';
...
// （woodJambColumnProbe.mjs等と同じ差し替え。openingStructuralSyncの既定recompute
// ＝recomputeActiveStructuralは自階のみを対象とするため、本probeでは実質未使用だが、
// 将来recomputeが下階peekを要する経路に変わっても壊れないよう他probeと同じ形で備える）。
...
// openingStructuralSyncを経由せず何か別経路が動いている（回帰の兆候）ため明示的に確認する。
const stillThereBeforeSync = !!findColumnNear(target);
if (!stillThereBeforeSync) {
  console.log('NG（対照）: request()を呼ぶ前から柱が消えている（openingStructuralSync以外の経路が動いている疑い）');
  process.exitCode = 1;
}
...
if (!process.exitCode) {
  console.log('OK: 建具の確定・削除直後にopeningStructuralSync経由で自階の構造が自動再計算される');
}
