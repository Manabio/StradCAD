...
// 自階（アクティブ平面）だけ厳密判定し、他平面は情報表示（centerMoveと同じ裁定。2026-09-25）----
setCenterLineStructuralListener((g, p, scope) => h.probeSync.request(g, p, { scope }));
...
if (undoDiffsOther.length > 0) {
  console.log(`M7(情報・他平面は非NG): 他平面のundo後ダンプ（構造フィールド）と基準の差分 ${undoDiffsOther.length}件（段階(g)まで許容）`);
  printDiffs(undoDiffsOther);
} else {
  console.log('M7(情報): 他平面のundo後ダンプ（構造フィールド）は基準と差分0件');
}
